...
def do_test(args, cfg, model):
    ...
    print(f"cats : {cats}")
    print(f"list_of_ims : {list_of_ims}")
    for path in list_of_ims:
        ...
        K = np.array(
            [[focal_length, 0.0, px], [0.0, focal_length, py], [0.0, 0.0, 1.0]]
        )
        ...
        print(type(K))
        if len(meshes) > 0:
            im_drawn_rgb = vis.draw_scene_view(
                im,
                K,
                meshes,
                text=meshes_text,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input-folder",
        type=str,
        help="list of image folders to process",
        required=True,
    )
    parser.add_argument(
        "--focal-length",
