...
def prepare_for_dn(
    ...
    tgt = (
        torch.cat([tgt_weight, indicator0], dim=1)
        + label_enc.weight[0][0] * torch.tensor(0).cuda()
    )
    ...
    if training:
        ...
        input_query_bbox = torch.cat([padding_bbox, refpoint_emb], dim=0).repeat(
            batch_size, 1, 1
        )
...
def prepare_for_loss(mask_dict):
    ...
    if len(output_known_class) > 0:
        ...
        output_known_3d_dim = output_known_3d_dim.permute(1, 2, 0, 3)[
            (bid, map_known_indice)
        ].permute(1, 0, 2)
        output_known_depth = output_known_depth.permute(1, 2, 0, 3)[
            (bid, map_known_indice)
        ].permute(1, 0, 2)
        output_known_angle = output_known_angle.permute(1, 2, 0, 3)[
            (bid, map_known_indice)
        ].permute(1, 0, 2)
...
def tgt_loss_depths(src_depth, tgt_depth, num_tgt):
    ...
    depth_loss = (
        1.4142 * torch.exp(-depth_log_variance) * torch.abs(depth_input - tgt_depth)
        + depth_log_variance
    )
    losses = {}
...
def tgt_loss_labels(src_logits_, tgt_labels_, num_tgt, focal_alpha, log=True):
    ...
    src_logits, tgt_labels = src_logits_.unsqueeze(0), tgt_labels_.unsqueeze(0).to(
        torch.int64
    )
    ...
    loss_ce = (
        sigmoid_focal_loss(
            src_logits, target_classes_onehot, num_tgt, alpha=focal_alpha, gamma=2
        )
        * src_logits.shape[1]
    )
...
def compute_dn_loss(mask_dict, training, aux_num, focal_alpha):
    ...
    if training and "output_known_lbs_bboxes" in mask_dict:
        ...
        losses.update(
            tgt_loss_labels(output_known_class[-1], known_labels, num_tgt, focal_alpha)
        )
